...
--============================================================================--
--
-- LuaSnip is a snippet engine for Neovim written in Lua. It is inspired by
-- UltiSnips and YASnippet.
--
-- In this custom configuration, we define a set of snippets for common comment
-- annotations such as FIXME, TODO, NOTE, HACK, etc. We also load snippets from
-- the snippets folder in the Neovim configuration directory.
--
-- The snippets are defined using the LuaSnip API and are added to the "all"
-- context. We also define a set of functions to get the current date, the
-- current user, and the comment string for the current file type.
--
-- The snippet definitions are then loaded using the LuaSnip loaders API.
--
--------------------------------------------------------------------------------
---@type LazySpec
return {
  ...
  config = function(plugin, opts)
    ...
    local function get_current_date() return os.date "%Y-%m-%d" end
